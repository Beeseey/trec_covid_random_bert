...
#### The root_folder variable in the read corpus function has to correspond to the directory your document_parses is in.

def read_paths(list_of_docids,metadata_dict):
...
def get_data():
	...
	metadata_dict = read_metadata('metadata_3.csv')

	rel_doc_dict = read_rel_judge('qrels-rnd1.txt')

	topic_dict = read_topics('topics-rnd1.xml')

	docids1 = read_docid_texts('docids-rnd1.txt')
	docids2 = read_docid_texts('docids-rnd2.txt')

	docids = [idx for idx in docids1 if idx not in docids2] + docids2

	keys = list(metadata_dict.keys())

	docids = [idx for idx in docids if idx in keys]

	corpus_dict = read_corpus(docids,metadata_dict,None)
